...
export default function StudentClassesPage() {
  ...
  const selectedClass = useMemo(() => classes.find((c) => c._id === selectedId) || null, [classes, selectedId]);

  function openSessions(c) {
    nav(`/student/classes/${c._id}/sessions`, { state: { classInfo: c } });
  }
  function openLeave(c) {
    nav(`/student/classes/${c._id}/leave`, { state: { classInfo: c } });
  }
                  ...
                  {PERIODS.map((p) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...classSlot,
                          outline: selectedId === c._id ? "2px solid rgba(120,120,255,0.55)" : "none",
                        }}
            ...
            {!selectedClass ? (
              ...
              <>
                <div style={{ marginTop: 14 }}>
                  ...
                  <div style={kvValue}>{selectedClass.subjectName} - {selectedClass.subjectCode}</div>
                </div>
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={kvValue}>
                    {DAYS.find((x) => x.value === selectedClass.dayOfWeek)?.label || selectedClass.dayOfWeek} — Ca {selectedClass.period}
                  </div>
                </div>
                <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <button type="button" style={btnPrimary} onClick={() => openSessions(selectedClass)}>Vào buổi điểm danh</button>
                  <button type="button" style={btnGhost} onClick={() => openLeave(selectedClass)}>Tạo đơn xin vắng</button>
                </div>
              </>
            )}
          </div>
...
const dayCard = { background: "#1b1b1b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.25)" };
...
const classSlot = { padding: 12, borderRadius: 10, border: "1px solid rgba(255,255,255,0.10)", background: "linear-gradient(180deg, rgba(255,255,255,0.04), rgba(255,255,255,0.02))", cursor: "pointer" };

const panel = { background: "#1b1b1b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 14, boxShadow: "0 10px 30px rgba(0,0,0,0.25)" };
...
const btnPrimary = { padding: "10px 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.12)", background: "#111", color: "#fff", cursor: "pointer", fontWeight: 900 };
const btnGhost = { padding: "10px 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.12)", background: "#2a2a2a", color: "#fff", cursor: "pointer", fontWeight: 900 };
